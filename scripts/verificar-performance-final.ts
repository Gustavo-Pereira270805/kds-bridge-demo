...
assert.ok(analyticsSource.includes('dentro_sla_cozinha'), 'KPI deve expor numerador nomeado de cozinha');
assert.ok(analyticsSource.includes('dentro_sla_salao'), 'KPI deve expor numerador nomeado de salão');
assert.ok(analyticsSource.includes('dentro_sla: parseInt(totals?.dentro_sla_cozinha'), 'alias numérico legado deve continuar equivalente à cozinha');
assert.ok(analyticsSource.includes('pct_dentro_sla_salao'), 'KPI deve expor percentual nomeado de salão');
assert.ok(analyticsSource.includes('sla_semantics'), 'payload deve documentar a semântica dos KPIs de SLA');
assert.ok(analyticsSource.includes('pct_dentro_sla é compatibilidade legada equivalente à cozinha'), 'alias legado deve documentar equivalência à cozinha');
assert.ok(analyticsSource.includes("status != 'annulled'"), 'métricas devem excluir demandas anuladas');
assert.ok(analyticsSource.includes("status NOT IN ('cancelled_salao','cancelled_cozinha')"), 'bases de SLA devem excluir cancelados');
...
assert.ok(productSection.includes("FROM demands WHERE ${dateFilter} AND status != 'annulled' AND status NOT IN ('cancelled_salao','cancelled_cozinha') AND sla_minutes IS NOT NULL AND ready_at IS NOT NULL"), 'SLA por produto deve usar a base elegível de cozinha');
assert.ok(productSection.includes("sla_breached_cozinha = true AND sla_minutes IS NOT NULL AND ready_at IS NOT NULL AND status NOT IN ('cancelled_salao','cancelled_cozinha')"), 'SLA por produto deve excluir cancelados do numerador');
...
assert.ok(breachSection.includes("$2::text IS NULL OR $2 = 'cozinha'") && breachSection.includes("$2::text IS NULL OR $2 = 'salao'"), 'responsible deve filtrar a consulta de estouros');
assert.ok(analyticsSource.includes("responsible deve ser cozinha ou salao"), 'responsible inválido deve ter mensagem pt-BR');
assert.ok(analyticsSource.includes("dentro_sla: 'dentro_sla é compatibilidade legada equivalente à cozinha"), 'alias numérico legado deve documentar equivalência à cozinha');
...
assert.ok(dashboardSource.includes("'SLA salão (%)'"), 'resumo deve exibir SLA do salão');
assert.ok(dashboardSource.includes("'Base SLA salão'"), 'resumo deve exibir base de SLA do salão');
assert.ok(dashboardSource.includes("'Dentro SLA salão'"), 'resumo deve exibir numerador de SLA do salão');
assert.ok(dashboardSource.includes("'SLA cozinha (%)'"), 'resumo deve exibir SLA da cozinha');
assert.ok(dashboardSource.includes("'Base SLA cozinha'"), 'resumo deve exibir base de SLA da cozinha');
assert.ok(dashboardSource.includes("'Dentro SLA cozinha'"), 'resumo deve exibir numerador de SLA da cozinha');
assert.ok(dashboardSource.includes('pct_dentro_sla_salao'), 'exportações devem usar o campo nomeado de salão');
assert.ok(analyticsSource.includes("Querystring: { days?: string; responsible?: string }"), 'endpoints relativos devem tipar days e responsible');
assert.ok(!analyticsSource.includes('(request.query as any)'), 'analytics não deve introduzir any para querystring');
